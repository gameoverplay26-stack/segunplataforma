...
`CombatSystem` expone en el Inspector del Player dos campos pensados para la demo, ambos apagados/en 0 por defecto: `Critical Chance` (probabilidad de golpe crítico x2 daño — en 0, cada disparo saca siempre `attackDamage` exacto, sin variación aleatoria) y `Enable Critical Hit Bug` (activarlo junto con `Critical Chance` > 0 reproduce en vivo, sobre la barra de vida del Enemy, la regresión real de CRYPT-201: el "crítico" duplica el golpe en vez de aumentarlo — mismo bug que ya narra la Sección F, ahora visible sin necesidad de leer el Test Runner).
